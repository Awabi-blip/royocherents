...
def main():
    time.sleep(0.5)
    print("="*135)
    print("="*65+"WELCOME TO ROYOCHERENTS"+"="*65)
    print("=" * 135)
    print("")
    action = input("Type C: to go to character selection or E to exit the game: ").strip().upper()
    if action == "E":
        close_game()
    elif action == "C":
                ...
                if sp_flag == True or ultimate_saved == True:
                    ...
                    if ult == "ULT":
                        ...
                        values = attack(player_health, player_attacks, player_damage, player_special, bot_health,bot_attacks,
                                        bot_damage, bot_special, player_name, bot_name, p_dmg, b_atk, b_dmg,j)
                        if len(values) == 3:
    ...
    if randints_bot_defend == 4:
        ...
        if randintsatk == 1:
            ...
            if player_health <= 0:
                ...
                time.sleep(1)
                print(f"{player_name} has been put an end to, Remaining HP : 0%! Well played. {bot_name} wins!")
                time.sleep(1)
                return None
            bot_health -= p_dmg
            if bot_health <= 0:
                print(f"{player_name} uses {p_atk} on {bot_name} dealing a {randexaggerations2} damage of {b_dmg}")
                time.sleep(1)
                print(f"{bot_name} has been put an end to! Well played. {player_name} wins!")
                time.sleep(1)
                return None
            else:
                print(f"{bot_name} uses {b_atk} on {player_name} dealing a {randexaggerations2} damage of {b_dmg}")
            ...
            if bot_health <= 0:
                ...
                time.sleep(1)
                print(
                    f"{bot_name} has been put an end to, Remaining HP : 0%! Well played. {player_name} wins!")
                time.sleep(1)
                return None
            player_health -= b_dmg
            if player_health <= 0:
                ...
                time.sleep(1)
                print(
                    f"{player_name} has been put an end to! Well played. {bot_name} wins!")
                time.sleep(1)
                return None
            else:
                print(f"{player_name} uses {p_atk} on {bot_name} dealing a {randexaggerations2} damage of {p_dmg}")
...
def dodge(dodger, attacker, atk, dmg, dodger_health, attacker_health, d_atk):
    ...
    if dodger_health <= 0:
        print(f"{dodger} has been put an end to! {attacker} wins!\nRemaining HP {attacker_health}")
        return None
    else:
    ...
    if player_special == "Terminal Atonement":
        ...
        for i in range(3):
            ...
            if bot_health <= 0:
                time.sleep(1)
                print(
                f"{bot_name} has been put an end to! Well played. {player_name} wins!")
                return None
        ...
        for i in range(5):
            ...
            if bot_health <= 0:
                time.sleep(1)
                print(
                f"{bot_name} has been put an end to! Well played. {player_name} wins!\nRemaining HP: {player_health}")
                return None

    player_health -= b_dmg
    print(f"{bot_name} uses {b_atk} on {player_name} dealing a {randexaggerations1} damage of {b_dmg}")
    if player_health <= 0:
        print(f"{player_name} has been put an end to! Well played. {bot_name} wins!\nRemaining HP: {bot_health}")
        return None

    health = (player_health, bot_health)
    return health

# def cpu_special(player_health, player_attacks, player_damage, player_special, bot_health, bot_attacks, bot_damage,
#                 bot_special, player_name, bot_name, p_dmg, b_atk, b_dmg):
#     if bot_special == "Terminal Atonement":
#         player_health -= 90
#         bot_health -= 25
#         print(f"Xeon uses TERMINAL ATONEMENT on {player_name} DEALING 90 damage\n")
#
#     elif bot_special == "Celestial Ascension":
#         player_health -= 60
#         bot_health -= 0
#         print(f"Rayquace uses Celestial Ascension on {bot_special} DEALING 60 damage\n")
#
#     elif bot_special == "METEORITE IMPACT":
#         player_health -= 60
#         bot_health -= 0
#         print(f"Zellyr uses METEORITE IMPACT on {bot_special} DEALING 60 damage\n")
#
#     elif bot_special == "Teleport":
#         print(f"Murkshaft dissappears into the shadows for 3 turns!!!\n")
#         for i in range(3):
#             player_health -= 15
#             bot_health += 5
#             time.sleep(1.5)
#             print(f"{player_name} gets hit by Murkshaft in a sneak attack losing 15 HP")
#             print("Murkshaft heals itself by 5HP\n")
#
#     elif bot_special == "Great Glacier Aieger":
#         print(f"{player_name} has been frozen for 5 turns!!!!\n")
#         for i in range(5):
#             player_health -= 10
#             bot_health -= 0
#             time.sleep(1.5)
#             print(f"The frost is too cold! {player_name} loses 10HP!\n")
#
#         health = (player_health, bot_health)
#         return health
